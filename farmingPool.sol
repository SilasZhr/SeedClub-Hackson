...
contract ERC1155Tradable is ERC1155, ERC1155MintBurn, ERC1155Metadata, Ownable, MinterRole, WhitelistAdminRole {
   ...
   * @param _from    The address to burn tokens from
   * @param _id      Token id to burn
   * @param _quantity  The amount to be burned
   */
	function burn(address _from, uint256 _id, uint256 _quantity) public  {
	    uint256 tokenId = _id;
	    address from = _from;
	    uint256 quantity = _quantity;
		  require(balanceOf(from , tokenId) >= quantity, "Insufficient balance");
...
contract NFTPool is ERC20TokenWrapper, Ownable {
  ...
  string poolName;
	...
	constructor(ERC1155Tradable _nftsAddress, IERC20 _erc20Address, string memory _poolName, uint256 _rate,uint256 _quotaLimit) public ERC20TokenWrapper(_erc20Address) {
		nfts = _nftsAddress;
    poolName = _poolName;
    rate = _rate;
    quotaLimit = _quotaLimit.mul(rate);
	}
	...
	// stake visibility is public as overriding Erc20TokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= quotaLimit, "Cannot stake more than quota limit");
